...
describe('Uploads Presign Routes', () => {
  ...
  it('debe rechazar token invalido con 401', async () => {
    const res = await request(app).post('/uploads/presign').set('Authorization', 'Bearer token_invalido').send({
      tipo: 'PRODUCTO',
      mimeType: 'image/jpeg',
    });
    expect(res.status).toBe(401);
  ...
  it('debe rechazar tipo de upload inválido con 400', async () => {
    const res = await request(app).post('/uploads/presign').set('Authorization', `Bearer ${tokenEmpleado}`).send({
      tipo: 'TIPO_INVALIDO',
      mimeType: 'image/jpeg',
    });
  ...
  it('debe rechazar si un cliente intenta subir imagen de producto con 403', async () => {
    const res = await request(app).post('/uploads/presign').set('Authorization', `Bearer ${tokenCliente}`).send({
      tipo: 'PRODUCTO',
      mimeType: 'image/jpeg',
    });
  ...
  it('debe rechazar mimeType no permitido para COMPROBANTE y PRODUCTO', async () => {
    const resComp = await request(app).post('/uploads/presign').set('Authorization', `Bearer ${tokenCliente}`).send({
      tipo: 'COMPROBANTE',
      mimeType: 'audio/mp3',
    });
    expect(resComp.status).toBe(400);

    const resProd = await request(app).post('/uploads/presign').set('Authorization', `Bearer ${tokenEmpleado}`).send({
      tipo: 'PRODUCTO',
